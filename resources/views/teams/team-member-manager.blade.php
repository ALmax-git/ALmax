      ...
      <x-form-section submit="addTeamMember">
        ...
        <x-slot name="form">
          ...
          <div class="col-span-6 sm:col-span-4">
            ...
            <x-input class="form-control" id="email" type="email" wire:model="addTeamMemberForm.email" />
          ...
          @if (count($this->roles) > 0)
            <div class="col-span-6 lg:col-span-4">
              ...
              <x-input-error class="mt-2" for="role" />

              <div class="relative z-0 mt-1 cursor-pointer rounded-lg border border-gray-200">
                @foreach ($this->roles as $index => $role)
                  <button
                    class="{{ $index > 0 ? 'border-t border-gray-200 focus:border-none rounded-t-none' : '' }} {{ !$loop->last ? 'rounded-b-none' : '' }} relative inline-flex w-full rounded-lg px-4 py-3 focus:z-10 focus:border-indigo-500 focus:outline-none focus:ring-2 focus:ring-indigo-500"
                    type="button" wire:click="$set('addTeamMemberForm.role', '{{ $role->key }}')">
                    <div
                      class="{{ isset($addTeamMemberForm['role']) && $addTeamMemberForm['role'] !== $role->key ? 'opacity-50' : '' }}">
                      <!-- Role Name -->
                      <div class="flex items-center">
                        <div
                          class="{{ $addTeamMemberForm['role'] == $role->key ? 'font-semibold' : '' }} text-sm text-gray-600">
                          {{ $role->name }}
                        </div>

                        @if ($addTeamMemberForm['role'] == $role->key)
                          <svg class="ms-2 size-5 text-green-400" xmlns="http://www.w3.org/2000/svg" fill="none"
              ...
              <div class="flex items-center justify-between">
                ...
                <div class="flex items-center">
                  @if (Gate::check('removeTeamMember', $team))
                    ...
                    <button class="ms-6 cursor-pointer text-sm text-red-500 focus:outline-none"
    ...
    <x-slot name="content">
      <div class="relative z-0 mt-1 cursor-pointer rounded-lg border border-gray-200">
        @foreach ($this->roles as $index => $role)
          <button
            class="{{ $index > 0 ? 'border-t border-gray-200 focus:border-none rounded-t-none' : '' }} {{ !$loop->last ? 'rounded-b-none' : '' }} relative inline-flex w-full rounded-lg px-4 py-3 focus:z-10 focus:border-indigo-500 focus:outline-none focus:ring-2 focus:ring-indigo-500"
            type="button" wire:click="$set('currentRole', '{{ $role->key }}')">
            <div class="{{ $currentRole !== $role->key ? 'opacity-50' : '' }}">
              <!-- Role Name -->
              <div class="flex items-center">
                <div class="{{ $currentRole == $role->key ? 'font-semibold' : '' }} text-sm text-gray-600">
                  {{ $role->name }}
                </div>
